...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'POST':
      return createAssetClass();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getAssetClasses() {
    const data = await prisma.asset_class
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'asset_class'));
  ...
  async function createAssetClass() {
    ...
    const data = await prisma.asset_class.create({
      data: body,
    });
    return res.status(200).json(data);
  }
