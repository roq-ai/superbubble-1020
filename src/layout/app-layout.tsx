...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'investment-analyst', 'portfolio-manager'];
  ...
  const { data, error, isLoading } = useSWR<FinTechInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/fin-teches` : null),
    () => getFinTeches({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Fin Tech',
      path: isTenantUser ? `/fin-teches/view/${data?.[0]?.id}` : '/fin-teches',
      entity: 'fin_tech',
